...
uint32 FAssetDataGatherer::Run()
{
	...
	static constexpr uint32 IntegrityCheckMagic = 0xCBA78340;
	...
	if ( bLoadAndSaveCache )
	{
		...
		if (FileAr && !FileAr->IsError() && FileAr->TotalSize() > 2 * sizeof(uint32))
		{
			...
			*FileAr << MagicNumber;
			...
			if (!FileAr->IsError() && MagicNumber == IntegrityCheckMagic)
			{
	...
	auto WriteAssetCacheFile = [&]()
	{
		...
		if (FileAr)
		{
			int32 MagicNumber = IntegrityCheckMagic;
			*FileAr << MagicNumber;
...
#if ALLOW_NAME_BATCH_SAVING
			{
				...
				FChecksumArchiveWriter ChecksummingWriter(*FileAr);
