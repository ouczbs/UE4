...
namespace Chaos
{
		...
		TPBDRigidParticleHandle<T, d>* CurrentNode)
	{
		...
		if (TPBDRigidClusteredParticleHandle<T, d>* ClusteredCurrentNode = CurrentNode->CastToClustered())
		{
			...
			{
				for (TPBDRigidParticleHandle<T, d>* Child : MChildren[ClusteredCurrentNode])
				{
	...
	void TPBDRigidClustering<T_FPBDRigidsEvolution, T_FPBDCollisionConstraint, T, d>::UpdateKinematicProperties(
		Chaos::TPBDRigidParticleHandle<float, 3>* InParent)
	{
		...
		if (TPBDRigidClusteredParticleHandle<T, d>* ClusteredCurrentNode = InParent->CastToClustered())
		{
			if (MChildren.Contains(ClusteredCurrentNode) && MChildren[ClusteredCurrentNode].Num())
			{
				...
				for (Chaos::TPBDRigidParticleHandle<float, 3>*Child : MChildren[ClusteredCurrentNode])
				{
					Queue.Enqueue(Child);
				}
				...
				{
					if (TPBDRigidClusteredParticleHandle<T, d>* CurrentClusterHandle = CurrentHandle->CastToClustered())
					{
						...
						{
							for (Chaos::TPBDRigidParticleHandle<float, 3>*Child : MChildren[CurrentClusterHandle])
							{
								Queue.Enqueue(Child);
							}
						}
...
*/
		...
		const T_FPBDCollisionConstraint& CollisionRule)
	{
		...
		for (const Chaos::FPBDCollisionConstraintHandle* ContactHandle : CollisionRule.GetConstConstraintHandles())
		{
			...
			if (const TArray<TPBDRigidParticleHandle<T, d>*>* ChildrenPtr = MParentToChildren.Find(ConstrainedParticles[0]->CastToClustered()))
			{
				ComputeStrainLambda(ConstrainedParticles[0]->CastToClustered(), *ChildrenPtr);
			}

			if (const TArray<TPBDRigidParticleHandle<T, d>*>* ChildrenPtr = MParentToChildren.Find(ConstrainedParticles[1]->CastToClustered()))
			{
