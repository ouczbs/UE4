...
namespace Chaos
{
	...
	void FHeightField::VisitTriangles(const FAABB3& QueryBounds, const TFunction<void(const TTriangle<FReal>& Triangle)>& Visitor) const
	{
		FBounds2D FlatQueryBounds;
		...
		for (const TVector<int32, 2>&Cell : Intersections)
		{
			...
			Visitor(TTriangle<FReal>(Points[0], Points[1], Points[3]));
			Visitor(TTriangle<FReal>(Points[0], Points[3], Points[2]));
		}
	}
