...
template<class T, int d>
TArray<TArray<int32>> TPBDLongRangeConstraintsBase<T, d>::ComputeIslands(
    const TMap<int32, TSet<int32>>& PointToNeighbors,
    ...
    const int32 MaxNumTetherIslands)
{
	...
	PhysicsParallelFor(Nodes.Num(), [&](int32 Index)
	{
		...
		for (const TArray<int32>& Elements : IslandElements)
		{
			...
			T ClosestSquareDistance = TNumericLimits<T>::Max();
			for (const int32 Element : Elements)
			{
				const T SquareDistance = (Particles.X(Element) - Particles.X(Node)).SizeSquared();
	...
	const int32 MaxNumTetherIslands)
{
	...
	for (const int32 Node : Nodes)
	{
		...
		if (Particles.InvM(Node) == (T)0.)
		{
			...
			for (const int32 Neighbor : Neighbors)
			{
				if (Particles.InvM(Neighbor) != (T)0.)  // There's no point to keep a node as an anchor, unless at least one of its neighbors is dynamic
				{
					Seeds.Add(Node);
					break;
				}
	...
	TMap<TVector<int32, 2>, T> GeodesicDistances;
	...
	for (const int32 Seed : Seeds)
	{
		for (const int32 Node : Nodes)
		{
			if (Particles.InvM(Node) != (T)0.)
			{
				GeodesicDistances.Emplace(TVector<int32, 2>(Seed, Node), Node != Seed ? TNumericLimits<T>::Max() : (T)0.);
			}
		}
	}
...
#define CHAOS_PBDLONGRANGEATTACHMENT_PROFILE_HEAPSIZE 0
		...
		auto LessPredicate = [Seed, &GeodesicDistances](int32 Node1, int32 Node2) -> bool
			{
				return GeodesicDistances[TVector<int32, 2>(Seed, Node1)] < GeodesicDistances[TVector<int32, 2>(Seed, Node2)];  // Less for node priority
			};
		...
		do
		{
			...
			for (const int32 NeighborNode : NeighborNodes)
			{
				...
				if (Particles.InvM(NeighborNode) == (T)0.)
				{
					continue;
				}

				// Update the geodesic distance if this path is a shorter one
				const T NewDistance = ParentDistance + (Particles.X(NeighborNode) - Particles.X(ParentNode)).Size();

				T& GeodesicDistance = GeodesicDistances[TVector<int32, 2>(Seed, NeighborNode)];
	...
	PhysicsParallelFor(Nodes.Num(), [&](int32 Index)
	{
		...
		for (const TArray<int32>& Seeds : Islands)
		{
			...
			for (const int32 Seed : Seeds)
			{
				const T Distance = GeodesicDistances[TVector<int32, 2>(Seed, Node)];
		...
		ClosestSeeds.Sort([&GeodesicDistances, Node](int32 Seed1, int32 Seed2)
			{
				return GeodesicDistances[TVector<int32, 2>(Seed1, Node)] < GeodesicDistances[TVector<int32, 2>(Seed2, Node)];
			});
		...
		{
			const T RefLength = GeodesicDistances[TVector<int32, 2>(Seed, Node)];
