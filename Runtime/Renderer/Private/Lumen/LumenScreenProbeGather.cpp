...
class FScreenProbeDownsampleDepthUniformCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture2D<float3>, RWScreenProbeWorldSpeed)
	...
	FRDGTextureRef& RoughSpecularIndirect)
{
	...
	if (View.ViewState)
	{
		...
		FRDGTextureRef NewDepthHistory = GraphBuilder.CreateTexture(SceneTextures.Depth.Resolve->Desc, TEXT("Lumen.ScreenProbeGather.DepthHistory"));
