...
void FDeferredShadingSceneRenderer::PrepareDistanceFieldScene(FRDGBuilder& GraphBuilder, bool bSplitDispatch)
{
	...
	if (bShouldPrepareDistanceFieldScene)
	{
		...
		const bool bLumenEnabled = GetViewPipelineState(Views[0]).DiffuseIndirectMethod == EDiffuseIndirectMethod::Lumen || GetViewPipelineState(Views[0]).ReflectionsMethod == EReflectionsMethod::Lumen;
		Scene->DistanceFieldSceneData.UpdateDistanceFieldAtlas(GraphBuilder, Views[0], Scene, bLumenEnabled, Views[0].ShaderMap, DistanceFieldAssetAdds, DistanceFieldAssetRemoves);
