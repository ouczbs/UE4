...
namespace ChaosTest {
	...
	void GraphDestroyParticles()
	{
		...
		TPBDPositionConstraints<float, 3> PositionConstraints;
		...
		TPBDRigidDynamicSpringConstraints<float, 3> SpringConstraints;
		for (const TVec2<int32>& Indices : ConstrainedParticles2)
		{
			FPBDRigidParticleHandle* Particle0 = AllParticles[Indices[0]]->CastToRigidParticle();
			FPBDRigidParticleHandle* Particle1 = AllParticles[Indices[1]]->CastToRigidParticle();
			auto* NewConstraint = SpringConstraints.AddConstraint(TPBDRigidDynamicSpringConstraints<float, 3>::FConstrainedParticlePair(Particle0, Particle1));
